...
export default function CourseEdit() {
  ...
  const fetchCourse = async() => {
    const data = await axios.get(`${api}/edit-course/${id}`);
    
    setValue("location", data.data.editCourse.location_id);
    
    await fetchCollege(data.data.editCourse.location_id);
    setValue("college", data.data.editCourse.college_id);

    await DepartmentList(data.data.editCourse.college_id);
    setValue("department", data.data.editCourse.department_id);

    setValue("year", Number(data.data.editCourse.year))

    const courseData = data.data.editCourse.course;
    setCourse(courseData.map(c=>({name:c})));
  }
  ...
  const onSubmit = async (data) => {
    try {
        const payload = {
            _id:id, ...data, courses:courses.map((c)=>c.name),
        }
      const result = await axios.post(`${api}/course-edit-submit`, payload);
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "40px", alignItems: "start"}}>
                ...
                render={({ field }) => (
                  <Select
                    ...
                    onChange={(selectedOption) => {
                      ...
                      DepartmentList(selectedOption.value)
                    }}
                    value={
            ...
            <div>
              ...
              <Controller
                ...
                render={({ field }) => (
                    ...
                    value={year.map((y)=>({value:y.value, label:y.label})).find((option) => option.value === field.value) || null}
                    styles={{
                        control: (base) => ({
                        ...base,
                        border: "1px solid rgba(255,255,255,0.2)",
            ...
            <div>
              ...
              {courses.map((course, index)=>(
                <div key={index} style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
                    <input type="text" 
                        ...
                        style={{ flex: 1, border: "1px solid rgba(0,0,0,0.2)", padding: "10px", borderRadius: "8px", fontSize: "16px", background: "rgba(255,255,255,0.9)", color: "#000"}}
                    />
                    {courses.length>1 && (
                        <button
                        type="button"
                        onClick={() => removeCourseField(index)}
                        style={{ backgroundColor: "red", color: "#fff", border: "none", borderRadius: "8px", padding: "0 12px", cursor: "pointer"}}
                    >
                      ×
                    </button>
                    )}
              ...
              <button
                ...
                style={{ backgroundColor: "#4CAF50", color: "#fff", border: "none", borderRadius: "8px", padding: "8px 12px", cursor: "pointer"}}
              >
                Add Course
              </button>
            </div>
