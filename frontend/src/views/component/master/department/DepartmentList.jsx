...
import {AddDepartmentbutton} from "../../../../utils/components/Addbutton";
...
export default function DepartmentList(){
    ...
    useEffect(()=>{
        fetchDepartmenttList();
    },[])

    const fetchDepartmenttList = async() => {
      try{
        const getDepartmentData = await axios.get(`${api}/department`);
        
        const data = getDepartmentData.data.department_list;
        
        
        const alldepartment = await Promise.all(
        data.map(async (item) => {
    ...
    const deleteCol = async(id)=>{
        const result = await Swal.fire({
          title:"Are You Sure",
          text:"You Want Delete Department",
          icon:"warning",
        ...
        if(result.isConfirmed){
          await axios.post(`${api}/delete-department/${id}`);
        ...
        {name:"Action",
          cell:(row)=>(
            <div style={{display:"flex", gap:"15px"}}>
              <FaEye
                title="View"
                onClick={()=>navigate(`/department/view/${row._id}`)}
                style={{
                  cursor:"pointer",
                  color:"#003cffff",
                  fontSize:"22px",
        ...
        <div
          ...
          <div style={{ marginTop: 120, height:"20%", marginLeft: 70, width: "75%", padding: 40, borderRadius: "20px", background: "rgba(243, 243, 243, 0.25)", boxShadow: "0 5px 18px 0 rgba(0, 0, 0, 0.37)"}}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px"}}>
              <h2 style={{ color: "#000000ff", fontWeight: 600 }}>Department Details</h2>
              <AddDepartmentbutton onClick={()=>navigate("/department/add")}/>
            </div>
